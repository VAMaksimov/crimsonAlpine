...
void *s21_memcpy(void *dest, const void *src, size_t n) {  // t
...
void *s21_memchr(const void *str, int c, size_t n) {  // t
  if (str == NULL) return NULL;
  char *p = (char *)str;
  for (n += 1; *p != '\0' && *p != c && n != 0; ++p, ++n);
  return *p == c ? p : NULL;
}

int s21_memcmp(const void *str1, const void *str2, size_t n) {  // t
...
void *s21_memset(void *str, int c, size_t n) {  // t
  char *s = (char *)str;
  for (size_t i = 0; i < n; ++i) s[i] = c;
  return str;
}

char *s21_strncat(char *dest, const char *src, size_t n) {  // t
  if (dest == NULL || src == NULL) return NULL;
  char *d = dest;
  const char *s = src;
  size_t d_len = s21_strlen(dest);
...
char *s21_strchr(const char *str, int c) {  // t
  if (str == NULL) return NULL;
  bool flag = false;
  size_t i = 0;
  while (str[i] != '\0' && !flag) {
  ...
  return flag ? (char *)(str + i) : NULL;
}

int s21_strncmp(const char *str1, const char *str2, size_t n) {  // t
  int res = 0;
...
char *s21_strncpy(char *dest, const char *src, size_t n) {  // t
  if (dest == NULL || src == NULL) return NULL;
  char *d = (char *)dest;
  const char *s = (const char *)src;
  for (size_t i = 0; i < n && s[i] != '\0'; ++i) {
...
size_t s21_strcspn(const char *str1, const char *str2) {  // t
...
char *s21_strpbrk(const char *str1, const char *str2) {  // t
...
char *s21_strrchr(const char *str, int c) {  // t
  if (str == NULL) return NULL;
  bool flag = false;
  const char *p = str;
  const char *res;
  ...
  return flag ? (char *)res : NULL;
}

char *s21_strstr(const char *haystack, const char *needle) {  // t
  ...
  for (; *p != '\0' && !flag; ++p) {
    if (*p == *needle && s21_strncmp(p, needle, len) == 0) {
      flag = true;
    }
  }
  return flag ? (char *)p : NULL;
}

char *strtok(char *str, const char *delim) {  // t
  size_t i = 0;
  char *s = NULL;
  static char *p = NULL;
...
void length_parser(const char *p, format_value *values) {
  values->length_exist = true;
  if (*p == 'l' && *(p + 1) == 'l') {
    ++p;
    values->length_value = ll;
  } else
    values->length_value = *p;
}

void specifier_parser(const char *p, format_value *values) {
  values->specifier_exist = true;
...
void format_flag_(char *buffer, size_t *index, format_value values, void *c,
                  ...
                  void (*write_to_buffer)(void *c, char *buffer, size_t *index,
                                          format_value values)) {
  char b = ' ';
  if (values.width_value > (int)len && values.flag_value != '-' &&
      values.flag_value != '0') {
    s21_memset(buffer + (*index), b, values.width_value - len);
    *index += values.width_value - len;
  }
  if (sign == '-' || values.flag_value == '+') buffer[(*index)++] = sign;
...
void formated_int(char *buffer, size_t *index, va_list factor,
                  format_value values) {
  long long v = 0;
  if (!values.length_exist) v = va_arg(factor, int);
  if (values.length_value == 'h') v = va_arg(factor, int);
  if (values.length_value == 'l') v = va_arg(factor, long);
  if (values.length_value == ll) v = va_arg(factor, long long);
  if (v != 0) {
    size_t len = 1;
    char sign = v < 0 ? '-' : '+';
...
void formated_uint(char *buffer, size_t *index, va_list factor,
  ...
  if (!values.length_exist) v = va_arg(factor, unsigned int);
  if (values.length_value == 'h') v = va_arg(factor, unsigned int);
  if (values.length_value == 'l') v = va_arg(factor, unsigned long);
  if (values.length_value == ll) v = va_arg(factor, unsigned long long);
  if (v != 0) {
    size_t len = 1;
    char sign = '+';
...
void formated_float(char *buffer, size_t *index, va_list factor,
                    format_value values) {
  long double v = 0;
  if (!values.length_exist) v = va_arg(factor, double);
  if (values.length_value == ll) v = va_arg(factor, long double);
...
int s21_sprintf(char *buffer, const char *format, ...) {
  ...
  while (!flag) {
    ...
    if (p == NULL || *p == '\0')
      ...
      p = format_parser(buffer, &index, ++p, factor);
    }
  }
  va_end(factor);
  buffer[index] = '\0';
